		...
		if (player != null) {
			if (!Minecraft.getInstance().isPaused()) {
				...
				if (Minecraft.getInstance().screen == null) {
					boolean useKeyAlsoActsAsThrow = ClientSetup.key_boththrow.getKey().equals(Minecraft.getInstance().options.keyUse.getKey());
					// keep in same order as enum from KeypressItem
					boolean keys[] = {ClientSetup.key_enderlaunch.isDown(), ClientSetup.key_leftthrow.isDown(), ClientSetup.key_rightthrow.isDown(), !useKeyAlsoActsAsThrow && ClientSetup.key_boththrow.isDown(), ClientSetup.key_rocket.isDown()};
