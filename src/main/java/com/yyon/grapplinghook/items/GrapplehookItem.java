...
public class GrapplehookItem extends Item implements KeypressItem {
	...
	public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
		...
		if (!level.isClientSide) {
			this.onCustomKeyDown(stack, player, KeypressItem.Keys.THROWBOTH, hand == InteractionHand.MAIN_HAND);
		}
	...
	public void releaseUsing(ItemStack stack, Level level, LivingEntity entityLiving, int timeLeft) {
		...
		if (this.getCustomization(stack).detachonkeyrelease) {
			Player player = (Player) entityLiving;
			this.onCustomKeyUp(stack, player, KeypressItem.Keys.THROWBOTH, player.getUsedItemHand() == InteractionHand.MAIN_HAND);
		}
	}
...
//  			verticalangle = custom.sneakingverticalthrowangle;
	...
	public void appendHoverText(ItemStack stack, @Nullable Level world, List<Component> list, TooltipFlag par4) {
		...
		String useKey = ClientProxyInterface.proxy.getKeyname(ClientProxyInterface.McKeys.keyBindUseItem);
		
		if (Screen.hasShiftDown()) {
			if (!custom.detachonkeyrelease) {
				list.add(Component.literal(useKey + " " + ClientProxyInterface.proxy.localize("grappletooltip.throw.desc")));
				list.add(Component.literal(useKey + " " + ClientProxyInterface.proxy.localize("grappletooltip.release.desc")));
				list.add(Component.literal(ClientProxyInterface.proxy.localize("grappletooltip.double.desc") + useKey + " " + ClientProxyInterface.proxy.localize("grappletooltip.releaseandthrow.desc")));
			} else {
				list.add(Component.literal(useKey + " " + ClientProxyInterface.proxy.localize("grappletooltip.throwhold.desc")));
			}
